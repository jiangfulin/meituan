/**
 * Created by mlamp on 2017/11/27.
 */
...
export const filterTap = (val,top,good) => {
  if(!val) return '';
  if(top) return '置顶';
  if(good) return '精华';
  if(val ==='share')return '分享';
  if(val ==='ask')return '问答';
  if(val ==='job')return '招聘';
  if(val ==='weex')return 'weex';
};
